...
## 📌 Features
- **Input Validation**: Sanitizes user input to prevent malicious characters and scripts.
- **Parameterized Queries**: Prevents SQL Injection attacks by using secure SQL commands.
- **XSS Protection**: Escapes user input before rendering to prevent script injection.
- **Authentication**: Secure login system with password hashing using **BCrypt**.
- **Role-Based Authorization (RBAC)**: Restricts access to sensitive features like the Admin Dashboard based on user roles.
- **Automated Security Tests**: NUnit test cases simulate SQL Injection, XSS attacks, and unauthorized access attempts.
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=YOUR_SERVER;Database=SafeVaultDB;User Id=YOUR_USER;Password=YOUR_PASSWORD;"
  }
}
```
---
